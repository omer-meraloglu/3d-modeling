#!/usr/bin/env python3
"""Blades of Chaos v4 — one-piece nominal metal casting master.

Run: python metal_model.py
...
The detailed casting master is exported as STL and 3MF. CadQuery also exports
a simplified, validated faceted STEP solid for CAD interchange.
"""
...
METAL_CORNER_RADIUS = .30         # XY tip/spike rounding for handling/finishing.
CASTING_PATTERN_SCALE = 1.000     # Finished nominal geometry. Foundry sets shrink.
CAD_FACET_TARGET = 65000          # CAD exchange tessellation; STL stays high-detail.
CAD_SAMPLE_ERROR_LIMIT = .10      # Measured sample distance acceptance, mm.
# ----------------------------------------------------------------------
...
def save_3mf(path,parts,names):
    ...
    chunks=['<?xml version="1.0" encoding="UTF-8"?>','<model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">','<metadata name="Title">Blades of Chaos v4 - one-piece metal casting master</metadata><resources>']
...
def save_mesh(path,item,stl=True):
    if stl:
        ...
        path.write_bytes(b'Blades of Chaos v4; mm; one-piece metal casting master'.ljust(80,b' ')+struct.pack('<I',len(f))+rows.tobytes())
...
def metal_mesh(pitch):
    factor=SINGLE_LENGTH/blade_polygon().bounds[2]
    for _ in range(5):
        s=Sculpt(factor)
        s.blade=s.blade.buffer(-METAL_CORNER_RADIUS).buffer(METAL_CORNER_RADIUS)
        s.guard=s.guard.buffer(-METAL_CORNER_RADIUS).buffer(METAL_CORNER_RADIUS)
    ...
    vertices*=CASTING_PATTERN_SCALE
    report,_=validate(vertices,faces,check_support=False)
    report['pattern_requires_support_planning']=not report.pop('support_check_passed')
    return {'vertices':vertices,'faces':faces,'material':mats,'top_count':len(top),'top_vertices':n,'report':report},s


def faceted_step(item,path,target):
    import trimesh,fast_simplification
    from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeVertex,BRepBuilderAPI_MakeEdge,BRepBuilderAPI_MakeFace,BRepBuilderAPI_MakeSolid
    from OCP.BRep import BRep_Builder
    from OCP.TopoDS import TopoDS_Wire,TopoDS_Shell,TopoDS
    from OCP.gp import gp_Pnt
    from OCP.BRepLib import BRepLib
    source=trimesh.Trimesh(item['vertices'],item['faces'],process=False)
    v,f=fast_simplification.simplify(source.vertices,source.faces,target_count=target,agg=5)
    coarse=trimesh.Trimesh(v,f,process=True)
    require(coarse.is_watertight and coarse.is_winding_consistent and coarse.is_volume,'Invalid simplified CAD shell.')
    # Reproducible bidirectional sampled surface-distance audit, not a claim of
    # a global Hausdorff bound. High-detail STL remains the production master.
    rng=np.random.default_rng(4021)
    indices=rng.choice(len(source.vertices),min(20000,len(source.vertices)),replace=False)
    _,d1,_=trimesh.proximity.closest_point(coarse,source.vertices[indices])
    points,_=trimesh.sample.sample_surface(coarse,10000,seed=407)
    _,d2,_=trimesh.proximity.closest_point(source,points)
    maxerr=float(max(d1.max(),d2.max()))
    require(maxerr<=CAD_SAMPLE_ERROR_LIMIT,f'CAD sample deviation {maxerr:.3f} mm exceeds limit.')
    print(f'CAD shell: {len(coarse.faces):,} faces, sampled deviation {maxerr:.4f} mm.',flush=True)
    builder=BRep_Builder();shell=TopoDS_Shell();builder.MakeShell(shell)
    verts=[BRepBuilderAPI_MakeVertex(gp_Pnt(*map(float,p))).Vertex() for p in coarse.vertices]
    shared={}
    for index,tri in enumerate(coarse.faces):
        wire=TopoDS_Wire();builder.MakeWire(wire)
        for a,b in zip(tri,np.roll(tri,-1)):
            key=(min(a,b),max(a,b))
            if key not in shared:shared[key]=BRepBuilderAPI_MakeEdge(verts[key[0]],verts[key[1]]).Edge()
            edge=shared[key] if a<b else TopoDS.Edge_s(shared[key].Reversed())
            builder.Add(wire,edge)
        wire.Closed(True)
        face=BRepBuilderAPI_MakeFace(wire,True).Face()
        expected=np.cross(coarse.vertices[tri[1]]-coarse.vertices[tri[0]],coarse.vertices[tri[2]]-coarse.vertices[tri[0]])
        actual=np.array(cq.Face(face).normalAt().toTuple())
        if actual@expected<0:face.Reverse()
        builder.Add(shell,face)
        if index and index%20000==0:print(f'CAD faces built: {index:,}',flush=True)
    shell.Closed(True);solid=cq.Solid(BRepBuilderAPI_MakeSolid(shell).Solid())
    BRepLib.SameParameter_s(solid.wrapped,1e-6,True)
    if not solid.isValid():
        print('Healing CAD shell topology...',flush=True)
        solid=solid.fix()
    require(solid.isValid() and len(solid.Solids())==1,'Invalid faceted STEP solid.')
    require(solid.Volume()>0,'Inverted CAD volume.')
    cq.exporters.export(solid,str(path))
    return {'type':'faceted B-rep solid; simplified interchange reference','faces':len(coarse.faces),
            'sampled_bidirectional_max_deviation_mm':maxerr,'sample_count':len(indices)+len(points),
            'cad_valid':True,'solids':1,'volume_cm3':solid.Volume()/1000,'authoritative_file':'metal_casting_master.stl'}
...
def main():
    ...
    p.add_argument('--skip-step',action='store_true');args=p.parse_args();SINGLE_LENGTH=args.length
    require(95<=SINGLE_LENGTH<=130,'Use 95-130 mm.')
    require(CASTING_PATTERN_SCALE==1.0,'Issue finished nominal master at 1:1; foundry owns compensation.')
    print('Building rounded one-piece metal master...',flush=True)
    item,s=metal_mesh(args.pitch)
    save_mesh(Path('metal_casting_master.stl'),item);save_3mf(Path('metal_casting_master.3mf'),[item],['One-piece casting; nominal finished size'])
    Path('output_model.stl').write_bytes(Path('metal_casting_master.stl').read_bytes())
    # Supplier outline, inspection/quote only: this is not a laser-cut flat part.
    import ezdxf
    doc=ezdxf.new('R2010');doc.units=4;space=doc.modelspace();yshift=-s.domain.bounds[1]
    for ring in [s.domain.exterior,*s.domain.interiors]:
        space.add_lwpolyline([(x,y+yshift) for x,y in ring.coords],close=True)
    doc.saveas('metal_outline_REFERENCE_ONLY.dxf')
    report={'version':4,'process_target':'investment casting from printed wax/castable resin master','nominal_scale':1.0,
       'rounding_xy_mm':METAL_CORNER_RADIUS,'mesh':item['report'],'minimum_edge_thickness_mm':2*FOUNDATION,
       'keyring_hole_mm':KEYRING_HOLE_D,'no_alignment_pockets':True,'no_split_seam':True,
       'material_alloy':'Supplier to confirm casting brass/bronze or agreed alternative',
       'estimated_mass_brass_8_5_g_cm3':item['report']['volume_cm3']*8.5,
       'estimated_mass_aluminium_2_7_g_cm3':item['report']['volume_cm3']*2.7,
       'foundry_trial_completed':False}
    Path('metal-validation.json').write_text(json.dumps(report,indent=2))
    if not args.skip_step:
        report['step']=faceted_step(item,Path('metal_CAD_interchange.step'),CAD_FACET_TARGET)
        Path('metal-validation.json').write_text(json.dumps(report,indent=2))
