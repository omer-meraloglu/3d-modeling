#!/usr/bin/env python3
"""Blades of Chaos v3 — two-sided, keyed, bondable miniature.
...
relief is exported as STL and 3MF.
"""
...
FOUNDATION = 0.90                 # Each half's blunt edge / seam land thickness.
BLADE_RISE = 1.55                 # Blade crown above the half's edge.
...
KEY_DIAMETER = 2.40               # Loose, double-taper registration keys.
KEY_TIP_DIAMETER = 1.00           # Flat ends make each key printable upright.
KEY_HALF_HEIGHT = 0.70           # 45-degree key flank; total key length 1.4 mm.
KEY_CLEARANCE = 0.25              # Radial clearance: socket diameter = 2.9 mm.
SOCKET_ROOF_SLOPE = 1.08          # dz/dr; roof steeper than 45 degrees.
MIN_SOCKET_SKIN = 0.65            # Minimum remaining material above a socket.
SOCKET_CENTERS = [(16,0),(52,-2.5),(86,-4)] # Canonical XY positions; scale with length.
BONDLINE_GAP = 0.08               # Assembly preview only; 0 for flush/welded fit.
PLATE_SPACING = 6.0               # Minimum nominal spacing between print parts.
...
def build_half(pitch):
    sculpt=Sculpt(SINGLE_LENGTH/blade_polygon().bounds[2]);domain=sculpt.domain
    radius=KEY_DIAMETER/2+KEY_CLEARANCE;depth=radius*SOCKET_ROOF_SLOPE
    centers=np.array(SOCKET_CENTERS,dtype=float)*sculpt.scale
    circles=[Point(*p).buffer(radius,quad_segs=32) for p in centers]
    require(all(domain.buffer(-.8).covers(c) for c in circles),'Socket too close to outer edge.')
    bottom_domain=orient(domain.difference(unary_union(circles)),sign=1)
    xy,top=planar_mesh(domain,pitch);z,mats=sculpt.sample(xy)
    bxy,bottom=planar_mesh(bottom_domain,1.0,boundary_pitch=pitch)
    n=len(xy);m=len(bxy);vertices=np.vstack([np.c_[xy,z],np.c_[bxy,np.zeros(m)]])
    facets=[top,bottom[:,::-1]+n]
    materials=[np.rint(mats[top].mean(axis=1)).astype(np.int32),np.ones(len(bottom),np.int32)]
    outer_edges=edge_list(top);lookup={tuple(np.round(p,8)):n+i for i,p in enumerate(bxy)}
    side=[]
    for a,b in outer_edges:
        aa=lookup[tuple(np.round(xy[a],8))];bb=lookup[tuple(np.round(xy[b],8))]
        side.extend([(a,aa,bb),(a,bb,b)])
    facets.append(np.array(side,np.int32));materials.append(np.repeat(mats[outer_edges[:,0]],2))
    inner_edges=edge_list(bottom)
    for center in centers:
        at=len(vertices);vertices=np.vstack([vertices,np.r_[center,depth]])
        ring=[(n+a,n+b,at) for a,b in inner_edges if np.linalg.norm((bxy[a]+bxy[b])/2-center)<radius+.01]
        require(len(ring)>=64,'Incomplete registration socket ring.')
        facets.append(np.array(ring,np.int32));materials.append(np.ones(len(ring),np.int32))
    faces=np.vstack(facets);material=np.concatenate(materials)
    minimum_skin=1e9
    for center in centers:
        xx,yy=np.meshgrid(np.linspace(-radius,radius,51),np.linspace(-radius,radius,51))
        local=np.c_[xx.ravel(),yy.ravel()];local=local[np.linalg.norm(local,axis=1)<=radius]
        h,_=sculpt.sample(local+center);roof=(radius-np.linalg.norm(local,axis=1))*SOCKET_ROOF_SLOPE
        minimum_skin=min(minimum_skin,float(np.min(h-roof)))
    require(minimum_skin>=MIN_SOCKET_SKIN,f'Socket skin {minimum_skin:.3f} mm is too thin.')
    ...
    envelope=wire.extrude(float(z.max())).val()
    for p in centers:envelope=envelope.cut(cq.Solid.makeCone(radius,0,depth,cq.Vector(float(p[0]),float(p[1]),0)))
    require(envelope.isValid() and len(envelope.Solids())==1,'Invalid CAD socket envelope.')
    shift=np.array([-domain.bounds[0],-domain.bounds[1],0.0]);vertices+=shift;centers+=shift[:2]
    report,_=validate(vertices,faces)
    report.update({'minimum_sampled_socket_skin_mm':minimum_skin,'socket_diameter_mm':2*radius,
       'socket_depth_mm':depth,'socket_roof_angle_deg':float(np.degrees(np.arctan(SOCKET_ROOF_SLOPE))),
       'key_clearance_per_side_mm':KEY_CLEARANCE,'key_centers_xy_mm':centers.tolist(),
       'cad_envelope_valid':True,'physical_print_tested':False})
    return {'vertices':vertices,'faces':faces,'material':material,'top_count':len(top),
            'top_vertices':n,'report':report,'centers':centers,'sculpt':sculpt,'shift':shift}


def mirror_half(front):
    out={k:v.copy() if hasattr(v,'copy') else v for k,v in front.items()}
    out['vertices'][:,0]=SINGLE_LENGTH-out['vertices'][:,0];out['faces']=out['faces'][:,::-1].copy()
    out['centers'][:,0]=SINGLE_LENGTH-out['centers'][:,0]
    out['report'],_=validate(out['vertices'],out['faces'])
    out['report']['mirror']='X; flip around Y to mate with A.'
    return out


def alignment_key():
    count=96;theta=np.arange(count)*2*pi/count
    rings=[np.c_[r*np.cos(theta),r*np.sin(theta),np.full(count,z)] for r,z in
       [(KEY_TIP_DIAMETER/2,0),(KEY_DIAMETER/2,KEY_HALF_HEIGHT),(KEY_TIP_DIAMETER/2,2*KEY_HALF_HEIGHT)]]
    verts=np.vstack(rings);faces=[]
    for layer in (0,1):
        for j in range(count):
            a=layer*count+j;b=layer*count+(j+1)%count;c=b+count;d=a+count;faces.extend([(a,b,c),(a,c,d)])
    verts=np.vstack([verts,[0,0,0],[0,0,2*KEY_HALF_HEIGHT]])
    for j in range(count):
        k=(j+1)%count;faces.extend([(3*count,k,j),(3*count+1,2*count+j,2*count+k)])
    faces=np.array(faces,np.int32);report,_=validate(verts,faces)
    return {'vertices':verts,'faces':faces,'material':np.full(len(faces),3),'top_count':0,'top_vertices':0,'report':report}


def moved(item,offset):return {**item,'vertices':item['vertices']+np.array(offset)}


def combined(parts):
    verts=[];faces=[];mats=[];offset=0
    for p in parts:
        verts.append(p['vertices']);faces.append(p['faces']+offset);mats.append(p['material']);offset+=len(p['vertices'])
    return {'vertices':np.vstack(verts),'faces':np.vstack(faces),'material':np.concatenate(mats),'top_count':0,'top_vertices':0}


def save_3mf(path,parts,names):
    from xml.sax.saxutils import escape
    chunks=['<?xml version="1.0" encoding="UTF-8"?>','<model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">','<metadata name="Title">Blades of Chaos v3</metadata><resources>']
...
def save_mesh(path,item,stl=True):
    if stl:
        ...
        path.write_bytes(b'Blades of Chaos v3; mm; keyed half'.ljust(80,b' ')+struct.pack('<I',len(f))+rows.tobytes())
...
def main():
    ...
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--length',type=float,default=SINGLE_LENGTH);parser.add_argument('--pitch',type=float,default=MESH_PITCH)
    args=parser.parse_args();SINGLE_LENGTH=args.length
    require(95<=SINGLE_LENGTH<=130,'Use 95-130 mm; socket skin is checked after customization.')
    require(.08<=args.pitch<=.25,'Use surface pitch .08-.25 mm.')
    require(.2<=KEY_CLEARANCE<=.3,'Use .20-.30 mm radial key clearance.')
    require(SOCKET_ROOF_SLOPE>=1.04,'Socket roofs must exceed 45 degrees.')
    require(KEY_HALF_HEIGHT>=(KEY_DIAMETER-KEY_TIP_DIAMETER)/2-1e-8,'Key overhang exceeds 45 degrees.')
    require(0<=BONDLINE_GAP<=.20,'Assembly gap must be 0-.20 mm.')
    folder=OUTPUT.parent;folder.mkdir(parents=True,exist_ok=True)
    print(f'Sculpting {SINGLE_LENGTH:.1f} mm front and mirrored back...',flush=True)
    front=build_half(args.pitch);back=mirror_half(front);key=alignment_key()
    for name,item in [('blade_A_front',front),('blade_B_back_mirrored',back),('alignment_key',key)]:
        save_mesh(folder/(name+'.stl'),item);save_3mf(folder/(name+'.3mf'),[item],[name])
    height=np.ptp(front['vertices'][:,1]);parts=[];names=[]
    for i,item in enumerate([front,back]):
        parts.append(moved(item,[0,i*(height+PLATE_SPACING),0]));names.append('A front' if i==0 else 'B mirrored back')
    for i in range(5):
        parts.append(moved(key,[8+i*5,2*(height+PLATE_SPACING),0]));names.append(f'Alignment key {i+1}'+(' spare' if i>=3 else ''))
    plate=combined(parts);plate_report,_=validate(plate['vertices'],plate['faces'],shells=7)
    save_mesh(OUTPUT,plate);save_3mf(OUTPUT.with_suffix('.3mf'),parts,names)
    pairs=[];pairnames=[]
    for i,item in enumerate([front,back,front,back]):
        pairs.append(moved(item,[0,i*(height+PLATE_SPACING),0]));pairnames.append(f'Blade {i//2+1} '+('A front' if i%2==0 else 'B back'))
    for i in range(8):
        pairs.append(moved(key,[8+i*5,4*(height+PLATE_SPACING),0]));pairnames.append(f'Alignment key {i+1}')
    save_3mf(folder/'two_blades_print_plate.3mf',pairs,pairnames)
    a=moved(front,[0,0,BONDLINE_GAP/2])
    b={**back,'vertices':back['vertices']*np.array([-1,1,-1])+np.array([SINGLE_LENGTH,0,-BONDLINE_GAP/2])}
    assembly=[a,b]+[moved(key,[*c,-KEY_HALF_HEIGHT]) for c in front['centers']]
    save_3mf(folder/'assembled_REFERENCE_ONLY.3mf',assembly,['A front','B back']+[f'Key {i+1}' for i in range(3)])
    save_mesh(folder/'assembled_REFERENCE_ONLY.stl',combined(assembly),stl=False)
    report={'version':3,'length_mm':SINGLE_LENGTH,'mesh_pitch_mm':args.pitch,'front_half':front['report'],'back_half':back['report'],
       'alignment_key':key['report'],'one_blade_print_plate':plate_report,'assembly_dimensions_mm':np.ptp(combined(assembly)['vertices'],axis=0).tolist(),
       'bondline_gap_preview_mm':BONDLINE_GAP,'halves_per_blade':2,'keys_per_blade':3,'spare_keys_per_plate':2,'two_blades_plate_parts':12,'physical_print_tested':False}
    (folder/'validation.json').write_text(json.dumps(report,indent=2))
    print(json.dumps({'assembly_mm':report['assembly_dimensions_mm'],'min_socket_skin_mm':front['report']['minimum_sampled_socket_skin_mm'],'output':str(OUTPUT)},indent=2),flush=True)
